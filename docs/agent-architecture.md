...
RabbitMQ ──► Agent System ──► SIEM Dashboard (план анализа)
  ...
  (Small LLM)  (VectorStore) (Large LLM)
```
...
class AgentPipeline:
    async def process(self, alert: NormalizedAlert) -> AnalysisPlan:
        ...
        # 2. Поиск плейбуков (семантический поиск с MMR)
        playbooks = self.vector_store.search(
            category=category.category,
            query=alert.rule_name,
            alert_message=alert.message,  # обогащённый запрос
        )

        # Fallback: если векторное хранилище пусто, берём из CATEGORY_PLAYBOOK_MAP
...
- Плейбуки разбиваются на чанки (MarkdownHeaderTextSplitter + RecursiveCharacterTextSplitter) для точного поиска
- Используется MMR (Max Marginal Relevance) для разнообразия результатов
...
- **Playbooks**: до 5 релевантных чанков из векторного хранилища (либо fallback из `CATEGORY_PLAYBOOK_MAP`)
...
Старт
  ...
  ├─► VectorStore(persist_dir, collection, ollama_base_url, embedding_model)
  │
  ├─► Если хранилище пусто → загрузить ALL_PLAYBOOKS (с разбивкой на чанки)
  │
...
| `CHROMA_PERSIST_DIR` | ./chroma_data | Директория для локального Chroma (SQLite) |
| `CHROMA_COLLECTION` | aisoc_playbooks | Имя коллекции в Chroma |
| `EMBEDDING_MODEL` | nomic-embed-text | Модель эмбеддингов в Ollama |
| `AGENT_HOST` | 0.0.0.0 | Хост FastAPI |
| `AGENT_PORT` | 8001 | Порт FastAPI |
| `VERBOSE` | true | Логирование этапов в терминал |
| `DB_PATH` | data/analyses.db | Путь к SQLite для сохранения результатов |
